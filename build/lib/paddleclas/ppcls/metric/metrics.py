...
class Recall(nn.Layer):
    def __init__(self, topk=(1,)):
        ...
        self.topk = topk
        if 5 in self.topk:
            raise NameError("the topk5 current is not supported")
    
    def _compute_recall(self, x, label, k):
        _, pred = paddle.topk(x, k=k, axis=1, largest=True, sorted=True)
        label = label.flatten()
        pred = pred.flatten()
        return paddle.to_tensor(recall_score(y_pred=pred, y_true=label, average='weighted'))
    ...
    def forward(self, x, label):
        ...
        metric_dict = dict()
        for k in self.topk:
            metric_dict["recall{}".format(k)] = self._compute_recall(x, label, k=k)
        return metric_dict
