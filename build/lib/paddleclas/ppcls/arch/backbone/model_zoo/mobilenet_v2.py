...
class ChannelGate(nn.Layer):
    ...
    def forward(self, x):
        ...
        for pool_type in self.pool_types:
            if pool_type=='avg':
                channel_att_raw = F.avg_pool2d( x, (x.shape[2], x.shape[3]), stride=(x.shape[2], x.shape[3]))
                # channel_att_raw = self.mlp( avg_pool )
            elif pool_type=='max':
                channel_att_raw = F.max_pool2d( x, (x.shape[2], x.shape[3]), stride=(x.shape[2], x.shape[3]))
                # channel_att_raw = self.mlp( max_pool )
            elif pool_type=='lp':
                channel_att_raw = F.lp_pool2d( x, 2, (x.shape[2], x.shape[3]), stride=(x.shape[2], x.shape[3]))
                # channel_att_raw = self.mlp( lp_pool )
            elif pool_type=='lse':
                # LSE pool only
                channel_att_raw = logsumexp_2d(x)
